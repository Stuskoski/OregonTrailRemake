...
public class CalculateDeathChancePerTurn {
    ...
    public static void CalculateDeaths(){
        if(Spouse.isAlive()){
            if(calcDidIDie(Profile.getSpouse().getChanceOfDying(), Profile.getLuck())){
                Spouse.setIsAlive(false);
                System.out.println(Spouse.getName()+" has died.");
            }
        }
        if(Child1.isAlive()){
            if(calcDidIDie(Profile.getChild1().getChanceOfDying(), Profile.getLuck())){
                Child1.setIsAlive(false);
                System.out.println(Child1.getName()+" has died.");
            }
        }
        if(Child2.isAlive()){
            if(calcDidIDie(Profile.getChild2().getChanceOfDying(), Profile.getLuck())){
                Child2.setIsAlive(false);
                System.out.println(Child2.getName()+" has died.");
            }
        }
        if(Child3.isAlive()){
            if(calcDidIDie(Profile.getChild3().getChanceOfDying(), Profile.getLuck())){
    ...
    private static boolean calcDidIDie(Double chance, int luck){
        double low = 0.00;
